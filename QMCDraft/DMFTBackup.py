...
'''
global variable set by Nico
this is set to true, if error < deltaSigma, since then DMFT is converged
'''
globalBreakSinceConverged = False
...
for iter_no in range(total_iterations + 1):
    ...
    print("globalBreakSinceConverged {}".format(globalBreakSinceConverged))
    ...
    #finished since last iteration           
    else:
        iter_type = "finish"
        iter_no = None
        log("Completing last self-consisting cycle ...")
    #here we reached the Convergence set in hdfout.py
    if globalBreakSinceConverged:
        iter_type = "finish"
        iter_no = None
        log("Completing last self-consisting cycle after Convergence ...") 

    print("iter type = {}".format(iter_type))
    ...
    if iter_type == "dmft" or iter_type == "stat":
        ...
        for iimp, imp_problem in enumerate(dmft_step.imp_problems):
            ...
            #This is Nicos Code to break, if DMFT is converged
            #____________________________________________________________________________________________________________
            #now also try to output siw directly
            numberOfFrequencies = 15
            middleFrequency = 1000           #Niw/2 as in Parameters.in
            if iter_no == 0:
                Value = np.zeros((total_iterations, numberOfFrequencies),dtype = complex)
                Error = np.zeros((total_iterations, numberOfFrequencies),dtype = complex)

            # write quantities not contained in result.other after
            # pulling band and spin dimensions to the front
            # get values of siw-full only, but also write the others into hdf5
            siwValueOutput, siwErrorOutput, rank0 = output.write_impurity_result_with_siw(
                iimp,
                {'siw-full': result.siw
                 ...
                 .apply(lambda x: np.transpose(x, (1, 2, 3, 4, 0)), inplace=True),
                 }, numberOfFrequencies, middleFrequency)

            #print("should be value and error and MasterRank {} {} {}".format(siwValueOutput, siwErrorOutput, rank0))
            #write output into array
            if rank0:
                for w in range(numberOfFrequencies):
                    Value[iter_no, w] = siwValueOutput[w]
                    Error[iter_no, w] = siwErrorOutput[w]
                #break if error smaller than difference
                threshold = 0
                #check if all SelfEnergies are below the error
                for w in range(numberOfFrequencies):
                    if iter_no > 0:
                        #calculate difference with Betragsquadrat
                        betragsQuadratOfDifferenz = (np.imag(Value[iter_no - 1, w]) - np.imag(Value[iter_no, w]))**2 + (np.real(Value[iter_no - 1, w]) - np.real(Value[iter_no, w]))**2 
                        print("Betragsquadrat {} and Fehler {}".format(np.sqrt(betragsQuadratOfDifferenz), np.real(Error[iter_no, w])))
                        if(np.sqrt(betragsQuadratOfDifferenz) < 0.000000000000000000000000000000000001):           #set very low, if you do not want this convergence
                            threshold = threshold + 1
                            print("threshold {}".format(threshold))
                    
                #here we end the programm, if error < deltaSigma
                if threshold == numberOfFrequencies:
                    print("convergence of the first {} frequencies reached".format(numberOfFrequencies))
                    globalBreakSinceConverged = True

            #At last send globalBreakSinceConverged = True to all other processes so the whole job can exit and not only rank0
            globalBreakSinceConverged = mpi_comm.bcast(globalBreakSinceConverged, root=0)
            print("globalBreakSinceConverged right after communication {}".format(globalBreakSinceConverged))
