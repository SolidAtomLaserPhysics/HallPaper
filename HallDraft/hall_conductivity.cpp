...
void genEnMat(Complex* matrix, int fluxNum, int fluxDen, double kx, double ky){ 
  ...
  if (L == 4){
    matrix[0] = 2.0*t*cos(kx) + 2.0*tPrimePrime*cos(2.0*kx);
    matrix[1] = t + 2.0*tPrime*cos(kx + pi*fluxNum/ fluxDen);
    matrix[2] = tPrimePrime*(1.0+exp(I*double(fluxDen)*ky));
    matrix[3] = t*exp(I*double(fluxDen)*ky) + 2.0*tPrime*cos(kx - pi*fluxNum/ fluxDen)*exp(I*double(fluxDen)*ky);

    matrix[4] = t + 2.0*tPrime*cos(kx + pi*fluxNum/ fluxDen);
    matrix[5] = 2.0*t*cos(kx + 2.0*pi*fluxNum / fluxDen) + 2.0*tPrimePrime*cos(2.0*(kx + 2.0*pi*fluxNum / fluxDen));
    matrix[6] = t + 2.0*tPrime*cos(kx + 3.0*pi*fluxNum/ fluxDen);
    matrix[7] = tPrimePrime*(1.0+exp(I*double(fluxDen)*ky));

    matrix[8] = tPrimePrime*(1.0+exp(-I*double(fluxDen)*ky));
    matrix[9] = t + 2.0*tPrime*cos(kx + 3.0*pi*fluxNum/ fluxDen);
    matrix[10] = 2.0*t*cos(kx + 2.0*pi*2.0*fluxNum / fluxDen) + 2.0*tPrimePrime*cos(2.0*(kx + 4.0*pi*fluxNum / fluxDen));
    matrix[11] = t + 2.0*tPrime*cos(kx + 5.0*pi*fluxNum/ fluxDen);

    matrix[12] = t*exp(-I*double(fluxDen)*ky) + 2.0*tPrime*cos(kx - pi*fluxNum/ fluxDen)*exp(-I*double(fluxDen)*ky);
    matrix[13] = tPrimePrime*(1.0+exp(-I*double(fluxDen)*ky));
    matrix[14] = t + 2.0*tPrime*cos(kx + 5.0*pi*fluxNum/ fluxDen);
    matrix[15] = 2.0*t*cos(kx + 2.0*pi*3.0*fluxNum / fluxDen) + 2.0*tPrimePrime*cos(2.0*(kx + 6.0*pi*fluxNum / fluxDen));
  }
}
...
void genVyMat(Complex* matrix, int fluxNum, int fluxDen, double kx, double ky){
  ...
  if (L == 4){
    matrix[0] = 0.0;
    matrix[1] = -I*t - 2.0*I*tPrime*cos(kx + pi*fluxNum/double(fluxDen));
    matrix[2] = 2.0*I*tPrimePrime*exp(I*ky*double(fluxDen)) - 2.0*I*tPrimePrime;
    matrix[3] = I*t*exp(I*ky*double(fluxDen)) + 2.0*I*tPrime*cos(kx - pi*fluxNum/double(fluxDen))*exp(I*ky*double(fluxDen));

    matrix[4] = I*t + 2.0*I*tPrime*cos(kx + pi*fluxNum/double(fluxDen));
    matrix[5] = 0.0;
    matrix[6] = -I*t - 2.0*I*tPrime*cos(kx + 3*pi*fluxNum/double(fluxDen));
    matrix[7] = 2.0*I*tPrimePrime*exp(I*ky*double(fluxDen)) - 2.0*I*tPrimePrime;

    matrix[8] = -2.0*I*tPrimePrime*exp(-I*ky*double(fluxDen)) + 2.0*I*tPrimePrime;
    matrix[9] = I*t + 2.0*I*tPrime*cos(kx + 3*pi*fluxNum/double(fluxDen));
    matrix[10] = 0.0;
    matrix[11] = -I*t - 2.0*I*tPrime*cos(kx + 5*pi*fluxNum/double(fluxDen));

    matrix[12] = -I*t*exp(-I*ky*double(fluxDen)) - 2.0*I*tPrime*cos(kx - pi*fluxNum/double(fluxDen))*exp(-I*ky*double(fluxDen));
    matrix[13] = -2.0*I*tPrimePrime*exp(-I*ky*double(fluxDen)) + 2.0*I*tPrimePrime;
    matrix[14] = I*t + 2.0*I*tPrime*cos(kx + 5*pi*fluxNum/double(fluxDen));
    matrix[15] = 0.0;
  }
}
...
void genVxMat(Complex* matrix, int fluxNum, int fluxDen, double kx, double ky){
  ...
  if (L == 3){
    ...
    matrix[8] = - 2.0*t*sin(kx + 4*pi*fluxNum/double(fluxDen)) - 4.0*tPrimePrime*sin(2.0*kx + 8*pi*fluxNum/double(fluxDen));  //sin or cos in second term?
  }
  if (L == 4){
    matrix[0] = - 2.0*t*sin(kx) - 4.0*tPrimePrime*sin(2.0*kx);
    matrix[1] = - 2.0*tPrime*sin(kx + pi*fluxNum/double(fluxDen));
    matrix[2] = 0.0;
    matrix[3] = - 2.0*tPrime*sin(kx - pi*fluxNum/double(fluxDen)) * exp(I*ky*double(fluxDen));

    matrix[4] = - 2.0*tPrime*sin(kx + pi*fluxNum/double(fluxDen));
    matrix[5] = -2.0*t*sin(kx + 2.0*pi*fluxNum/double(fluxDen)) - 4.0*tPrimePrime*sin(2.0*kx + 4*pi*fluxNum/double(fluxDen));
    matrix[6] = -2.0*tPrime*sin(kx + 3*pi*fluxNum/double(fluxDen));
    matrix[7] = 0.0;

    matrix[8] = 0.0;
    matrix[9] = - 2.0*tPrime*sin(kx + 3*pi*fluxNum/double(fluxDen)); 
    matrix[10] = - 2.0*t*sin(kx + 4*pi*fluxNum/double(fluxDen)) - 4.0*tPrimePrime*sin(2.0*kx + 8*pi*fluxNum/double(fluxDen));
    matrix[11] = - 2.0*tPrime*sin(kx + 5*pi*fluxNum/double(fluxDen)); 

    matrix[12] = - 2.0*tPrime*sin(kx - pi*fluxNum/double(fluxDen)) * exp(-I*ky*double(fluxDen));
    matrix[13] = 0.0;
    matrix[14] = - 2.0*tPrime*sin(kx + 5*pi*fluxNum/double(fluxDen)); 
    matrix[15] = - 2.0*t*sin(kx + 6*pi*fluxNum/double(fluxDen)) - 4.0*tPrimePrime*sin(2.0*kx + 12*pi*fluxNum/double(fluxDen));
  }
}
...
void genVxMatT(Complex* matrix, int fluxNum, int fluxDen, double kx, double ky){
  ...
  if (L == 3){
    ...
    matrix[6] = - 2.0*tPrime *sin(kx - pi*fluxNum/double(fluxDen))*exp(I*ky*double(fluxDen));
    matrix[7] = - 2.0*tPrime*sin(kx + 3*pi*fluxNum/double(fluxDen));
    matrix[8] = - 2.0*t*sin(kx + 4*pi*fluxNum/double(fluxDen)) - 4.0*tPrimePrime*sin(2.0*kx + 8*pi*fluxNum/double(fluxDen));
  }
  if (L == 4){
    matrix[0] = - 2.0*t*sin(kx) - 4.0*tPrimePrime*sin(2.0*kx);
    matrix[1] = - 2.0*tPrime*sin(kx + pi*fluxNum/double(fluxDen));
