...
void genVyMat(Complex* matrix, int fluxNum, int fluxDen, double kx, double ky){
  ...
  if (L == 3){
    matrix[0] = 0.0;
    matrix[1] = I*t + 2.0*I*tPrime*cos(kx + pi*fluxNum/double(fluxDen)) + 4.0*I*tPrimePrime*exp(I*ky*double(fluxDen));
    matrix[2] = 5.0*I*t*exp(I*ky*double(fluxDen)) + 10.0*I*tPrime*cos(kx - pi*fluxNum/double(fluxDen))*exp(I*ky*double(fluxDen)) + 2.0*I*tPrimePrime;

    matrix[3] = -I*t - 2.0*I*tPrime*cos(kx + pi*fluxNum/double(fluxDen)) - 4.0*I*tPrimePrime*exp(-I*ky*double(fluxDen));
    matrix[4] = 0.0;
    matrix[5] = I*t + 2.0*I*tPrime*cos(kx + 3*pi*fluxNum/double(fluxDen)) + 4.0*I*tPrimePrime*exp(I*ky*double(fluxDen));

    matrix[6] = -5.0*I*t*exp(-I*ky*double(fluxDen)) - 10.0*I*tPrime*cos(kx - pi*fluxNum/double(fluxDen))*exp(-I*ky*double(fluxDen)) - 2.0*I*tPrimePrime;
    matrix[7] = -I*t - 2.0*I*tPrime*cos(kx + 3*pi*fluxNum/double(fluxDen)) - 4.0*I*tPrimePrime*exp(-I*ky*double(fluxDen));
    matrix[8] = 0.0;
  }
}
