...
c            endif
     ...
     $              dreal(G0wmat(i,j,j)),dimag(G0wmat(i,j,j))
            ...
            do i=0,Iwmax
               do j=1,L
                  G0w(i)=G0w(i)+G0wmat(i,j,j)/dfloat(L)                                !sum over nu
               enddo
            enddo
...
!Construct dispersion matrix for given values of kx and ky
     ...
     $              dcmplx(0.0d0,-t*dsin(dfloat(L)*ky))
            ...
            if (L.eq.3) then
               !use p = 1 here,since use it in thesis
               epsmat(1,1) = 2.0*t*dcos(kx) + 2.0*t2*dcos(2.0*kx);
               epsmat(1,2) = t + 2.0*t1*dcos(kx + pi*1.0/ L) + t2*
     $              (dcmplx(dcos(dfloat(L)*ky),0.0d0)+
     $              dcmplx(0.0d0,dsin(dfloat(L)*ky)));
               epsmat(1,3) = t*dcmplx(dcos(dfloat(L)*ky),0.0d0)+
     $              t*dcmplx(0.0d0,dsin(dfloat(L)*ky)) + 
     $              2.0*t1*dcos(kx - pi*1.0/ L)*
     $              (dcmplx(dcos(dfloat(L)*ky),0.0d0)+
     $              dcmplx(0.0d0,dsin(dfloat(L)*ky))) + t2;
           
               epsmat(2,1) = t + 2.0*t1*dcos(kx + pi*1.0/ L) + 
     $              t2*(dcmplx(dcos(dfloat(-L)*ky),0.0d0)+
     $              dcmplx(0.0d0,dsin(dfloat(-L)*ky)));
               epsmat(2,2) = 2.0*t*dcos(kx + 2.0*pi*1.0 / L) + 
     $              2.0*t2*dcos(2.0*(kx + 2.0*pi*1.0 / L));
               epsmat(2,3) = t + 2.0*t1*dcos(kx + 3.0*pi*1.0/ L) + 
     $              t2*(dcmplx(dcos(dfloat(L)*ky),0.0d0)+
     $              dcmplx(0.0d0,dsin(dfloat(L)*ky)));
           
               epsmat(3,1) = dcmplx(t*dcos(dfloat(-L)*ky),0.0d0)+
     $              dcmplx(0.0d0,t*dsin(dfloat(-L)*ky)) + 
     $              2.0*t1*dcos(kx - pi*1.0/ L)*
     $              (dcmplx(dcos(dfloat(-L)*ky),0.0d0)+
     $              dcmplx(0.0d0,dsin(dfloat(-L)*ky)))+ t2;
               epsmat(3,2) = t + 2.0*t1*dcos(kx + 3.0*pi*1.0/ L) + 
     $              t2*(dcmplx(dcos(dfloat(-L)*ky),0.0d0)+
     $              dcmplx(0.0d0,dsin(dfloat(-L)*ky)));
               epsmat(3,3) = 2.0*t*dcos(kx + 2.0*pi*2.0*1.0 / L) + 
     $              2.0*t2*dcos(2.0*(kx + 4.0*pi*1.0 / L));
            endif

            if (L.eq.4) then
               !use p = 1 here,since use it in thesis
               epsmat(1,1) = 2.0*t*dcos(kx) + 2.0*t2*dcos(2.0*kx);
               epsmat(1,2) = t + 2.0*t1*dcos(kx + pi*1.0/ L);
               epsmat(1,3) = t2*(1+dcmplx(dcos(dfloat(L)*ky),0.0d0)+
     $              dcmplx(0.0d0,dsin(dfloat(L)*ky)));
               epsmat(1,4) = t*dcmplx(dcos(dfloat(L)*ky),0.0d0)+
     $              t*dcmplx(0.0d0,dsin(dfloat(L)*ky)) + 
     $              2.0*t1*dcos(kx - pi*1.0/ L)*
     $              (dcmplx(dcos(dfloat(L)*ky),0.0d0)+
     $              dcmplx(0.0d0,dsin(dfloat(L)*ky)));
     ...
     $              2.0*t2*dcos(2.0*(kx + 2.0*pi*1.0 / L));
               epsmat(2,3) = t + 2.0*t1*dcos(kx + 3.0*pi*1.0/ L);
               epsmat(2,4) = t2*(1+dcmplx(dcos(dfloat(L)*ky),0.0d0)+
     $              dcmplx(0.0d0,dsin(dfloat(L)*ky)));
           
               epsmat(3,1) = t2*(1+dcmplx(dcos(dfloat(-L)*ky),0.0d0)+
     $              dcmplx(0.0d0,dsin(dfloat(-L)*ky)));
               epsmat(3,2) = t + 2.0*t1*dcos(kx + 3.0*pi*1.0/ L);
               epsmat(3,3) = 2.0*t*dcos(kx + 2.0*pi*2.0*1.0 / L) + 
     $              2.0*t2*dcos(2.0*(kx + 4.0*pi*1.0 / L));
               epsmat(3,4) = t + 2.0*t1*dcos(kx + 5.0*pi*1.0/ L);

               epsmat(4,1) = t*dcmplx(dcos(dfloat(-L)*ky),0.0d0)+
     $              t*dcmplx(0.0d0,dsin(dfloat(-L)*ky)) + 
     $              2.0*t1*dcos(kx - pi*1.0/ L)*
     $              (dcmplx(dcos(dfloat(-L)*ky),0.0d0)+
     $              dcmplx(0.0d0,dsin(dfloat(-L)*ky)));
               epsmat(4,2) = t2*(1+dcmplx(dcos(dfloat(-L)*ky),0.0d0)+
     $              dcmplx(0.0d0,dsin(dfloat(-L)*ky)));
               epsmat(4,3) = t + 2.0*t1*dcos(kx + 5.0*pi*1.0/ L);
               epsmat(4,4) = 2.0*t*dcos(kx + 2.0*pi*3.0*1.0 / L) + 
     $              2.0*t2*dcos(2.0*(kx + 6.0*pi*1.0 / L));
            endif
