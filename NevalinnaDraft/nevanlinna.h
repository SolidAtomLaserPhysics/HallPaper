...
public:
    ...
    void write () {
        for(int i = 0;i < N_real_; i++){
            ...
            //this line, if I want to print A(w)
            //ofs << freq_[i].real() << " " << 1 / M_PI * val_[i].imag() <<std::endl;                 //change output from A to G^R ?
            //this line if I want to print G^R with real and imag part
            ofs << freq_[i].real() << " " << - val_[i].real() << " " << - val_[i].imag() <<std::endl;       //I think in Code it is minus greens function => need minus to redo it
        }
    }
private:
    std::ofstream ofs;
