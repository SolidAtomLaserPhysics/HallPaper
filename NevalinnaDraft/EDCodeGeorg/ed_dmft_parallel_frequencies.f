...
!Magnetic field parameter definition
      parameter (L=3)
      parameter (Bm=1.0d0/3.0d0)
      include'init.h'  
      parameter (Iwmax= 2**15)
      parameter (Iwmaxreal =2**1)
...
!         call  selfconst2_magnetic(omnumber,omoffset,Iwmax,Nkx,Nky,L, !Integer-Input
!     $                         beta,xmu,Bm,                          !Real-Input  
!     $                         om,                                  !Real-Array-Input
!     $                         Xi,                                   !Complex-Input      
!     $                         g0wand,Gw,                            !Complex-Array-Input
!     $                         G0wpart)                           !Complex-Array-Output


         call selfconst2_magneticfast(omnumber,omoffset,Iwmax, !Integer-Input
     $        Nkx,Nky,L,beta,xmu,Bm, !Real-Input  
     $        om,               !Real-Array-Input
     $        Xi,               !Complex-Input      
     $        g0wand,Gw,        !Complex-Array-Input
     $        G0wpart)             !Complex-Array-Output
...
!Here the local interacting (!) Green's function is written to G0w and G0wpart
         ...
         call MPI_BARRIER(MPI_COMM_WORLD,ierror)
         call MPI_GATHERV(
     $        G0wpart(omoffset+1:omoffset+omnumber),
     $        omnumber,
     $        MPI_DOUBLE_COMPLEX,G0w,
     $        recvnumber,offsetnum,
...
c            do j=1,L
c               diff(j)=dcmplx(0.0d0,0.0d0)
c            enddo
            
c            do i=0,Iwmax
c               do j=1,L
c               write(2000,'(3f20.13)')om(i),
c     $              dreal(G0w(i)),dimag(G0w(i))
c               enddo
c            enddo
            
c            do i=0,Iwmax
c               G0w(i)=G0wmat(i,1,1)
c               do j=2,L
c                  diff(j)=diff(j)+abs(G0w(i)-G0wmat(i,j,j))
c               enddo
c            enddo
c            do j=2,L
c               write(6,*)"Difference to orbital 1 for orbital",j,diff(j)
c            enddo

c            do i=0,Iwmax
c               G0w(i)=dcmplx(0.0d0,0.0d0)
c            enddo

c            do i=0,Iwmax
c               do j=1,L
c                  G0w(i)=G0w(i)+G0wmat(i,j,j)/dfloat(L)
c               enddo
c            enddo
...
!damping of self-consistency
               G0w(i)=0.50d0/G0w(i)+0.50d0*g0wand(i)
            enddo
         endif
     ...
     $                 /dfloat(5000)
      ...
      if (myid.eq.0) then
         ...
         do i=0,2*Iwmax-1
            ...
            write(90,'(3f55.50)') dimag(ni(2*i+1))
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
    ...
    1 continue
c      write(72,*)(nstato+1),(anew(i),i=1,ns)
      return
      end
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine selfconst2_magnetic(omnumber,omoffset,Iwmax,Nkx,Nky,L,   !Integer-Input
...
!Input-Variables
      integer omnumber,omoffset,Iwmax,Nkx,Nky,myblock,L
      real*8 beta,xmu,B
...
!one flux quantum Phi_0 goes through the unit cell: L=1/B
...
!Subroutine-Internal-Variables
      ...
      real*8 weightx,weighty,weightm,kx,ky,Pi
      real*8 t,t1,t2,checkone,dkx,dky
      complex*16 W(omoffset+1:omoffset+omnumber)
      complex*16 gand(omoffset+1:omoffset+omnumber)
      complex*16 Gw1(omoffset+1:omoffset+omnumber)
      complex*16 epsmat(1:L,1:L)
      complex*16 ginv(1:L,1:L)
      complex*16 gk
!Varaibles for inversion subroutines
      INTEGER infoinv
      INTEGER ipiv(1:L)
      COMPLEX*16 workinv(1:10*L)
      include 'tpri.dat'
      Pi=dacos(-1.0d0)
...
!      write(6,*)B
      ...
      do i=omoffset+1,omoffset+omnumber
         gand(i)=1.d0/g0wand(i)
         Gw1(i)=1.d0/Gw(i)
         W(i)= Xi*om(i)-gand(i)+Gw1(i)+xmu!-tpri
      enddo
      
      checkone=0.0d0

      dkx=2.0d0*pi/dfloat(Nkx)
      dky=2.0d0*pi/dfloat(L*Nky)

      do ikx=1,Nkx
         kx=-Pi+dfloat(ikx)*dkx
         do iky=1,Nky
            ky=-Pi/dfloat(L)+dfloat(iky)*dky
            checkone=checkone+1.0d0
!Construct dispersion matrix for given valuex of kx and ky
     ...
     $              dcmplx(0.0d0,-t*dsin(dfloat(L)*ky))
               ...
               epsmat(2,1)=dcmplx(-t-t*dcos(dfloat(L)*ky),0.0d0)-
     $              dcmplx(0.0d0,-t*dsin(dfloat(L)*ky))
            endif
            ...
            do i=omoffset+1,omoffset+omnumber         
               ...
               enddo
               do j=1,L
                  ginv(j,j)=ginv(j,j)+W(i)
               enddo
            
               CALL ZGETRF(L,L,ginv,L,ipiv,infoinv)
               CALL ZGETRI(L,ginv,L,ipiv,workinv,10*L,infoinv)
               
               gloc(i)=gloc(i)+ginv(1,1)
               
            enddo
         enddo
      enddo 

      checkone=checkone/dfloat(Nkx*Nky)
      if (omoffset.le.2) then
         write(6,*)"Check momentum sum: ",checkone
      endif

      do i=omoffset+1,omoffset+omnumber
         gloc(i)=gloc(i)/dfloat(Nkx*Nky)
      enddo
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine selfconst2_magneticfast(omnumber,omoffset,Iwmax,         !Integer-Input
     $                               Nkx,Nky,L,beta,xmu,B,                !Real-Input  
     $                               om,                                  !Real-Array-Input
     $                               Xi,                                  !Complex-Input      
     $                               g0wand,Gw,                           !Complex-Array-Input
     $                               Gloc)                                !Complex-Array-Output
...
!Output-Variables
      complex*16 gloc(0:Iwmax)
!Subroutine-Internal-Variables
      integer i,j,k,ikx,iky,ksteps
      real*8 weightx,weighty,weightm,kx,ky,Pi
      real*8 t,t1,t2,checkone,dkx,dky
      complex*16 W(omoffset+1:omoffset+omnumber)
      complex*16 gand(omoffset+1:omoffset+omnumber)
      complex*16 Gw1(omoffset+1:omoffset+omnumber)
      complex*16 epspart(1:L),subdiag1(1:L-1),subdiag2(1:L-1)
      complex*16 ginvpart(1:L),e1q(1:L,1:2)
      !Bad ky sampling
!     complex*16 kytab(1:2,1:2*((Nkx/2)/L))
      complex*16 kytab(1:2,1:Nky)
      complex*16 gk
!Varaibles for inversion subroutines
      INTEGER infoinv
      INTEGER ipiv(1:L)
...
!      write(6,*)B
      ...
      !Bad ky sampling!
!      ksteps=Nkx/2
!      do j=-ksteps/L+1,ksteps/L
!         ky=Pi*dfloat(j)/dfloat(ksteps)
!         kytab(1,j+ksteps/L)=dcmplx(dcos(dfloat(L)*ky),
!     $        dsin(dfloat(L)*ky))
!         kytab(2,j+ksteps/L)=dcmplx(dcos(dfloat(L)*ky),
!     $        -dsin(dfloat(L)*ky))
!      enddo
      
      do j=1,Nky
         ky=-pi/dfloat(L)+dfloat(j-1)*dky
         kytab(1,j)=dcmplx(dcos(dfloat(L)*ky),dsin(dfloat(L)*ky))
         kytab(2,j)=dcmplx(dcos(dfloat(L)*ky),-dsin(dfloat(L)*ky))
      enddo
      
      do i=0,Iwmax
         gloc(i)=dcmplx(0.0d0,0.0d0)
      enddo
            
      do ikx=1,Nkx                  
         kx=-Pi+dfloat(ikx)*dkx
         do j=1,L
            epspart(j)=dcmplx(-2.0d0*t*
     $           dcos(kx+dfloat(j-1)*B*2.0d0*Pi),0.0d0)
         enddo
         epspart(1)=epspart(1)+t
         epspart(L)=epspart(L)+t
         
         do i=omoffset+1,omoffset+omnumber         
            do j=1,L
               ginvpart(j)=W(i)-epspart(j)
               e1q(j,1)=dcmplx(0.0d0,0.0d0)
               e1q(j,2)=dcmplx(0.0d0,0.0d0)
            enddo
            e1q(1,1)=dcmplx(1.0d0,0.0d0)
            e1q(L,2)=dcmplx(1.0d0,0.0d0)
            do j=1,L-1
               subdiag1(j)=dcmplx(t,0.0d0)
               subdiag2(j)=dcmplx(t,0.0d0)
            enddo

            CALL ZGTSV(L,2,subdiag1,ginvpart,subdiag2,e1q,L,infoinv)
            
            !Bad ky sampling
!            do iky=1,2*(ksteps/L)
            do iky=1,Nky
               gloc(i)=gloc(i)+e1q(1,1)-
     $              t*(e1q(1,1)+e1q(1,2)*kytab(2,iky))*
     $              (e1q(1,1)+e1q(L,1)*kytab(1,iky))/
     $              (1.0d0+t*(e1q(1,1)+e1q(1,2)*kytab(2,iky)
     $              +e1q(L,1)*kytab(1,iky)+e1q(L,2)))
            enddo
         enddo
      enddo
      ...
      do i=omoffset+1,omoffset+omnumber
         !Bad ky sampling
!         gloc(i)=gloc(i)/dfloat(Nkx*2*(ksteps/L))
         gloc(i)=gloc(i)/dfloat(Nkx*Nky)
      enddo
