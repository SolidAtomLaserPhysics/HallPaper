program DensityOfStates
  ...
  real(real64), parameter :: t=0.25d0
  real(real64), parameter :: t1=0.0d0
  real(real64), parameter :: t2=0.0d0

  !Logical variables to determine what should be calculated.
  logical, parameter :: calcdos=.TRUE.
  logical, parameter :: calcdoslowrankupdate=.False.
  logical, parameter :: calcgretarded=.False.
  logical, parameter :: calcgretardedlowrankupdate=.False.
  logical, parameter :: calcgmatsubara=.False.
  logical, parameter :: calcgmatsubaralowrankupdate=.False.
     ...
     do i=k_min,k_max
        ...
        if (calcdos) then
           !Construct (negative) full dispersion matrix.
            dkx = (2 * Pi)/dfloat(Nkx)
            dky = (2 * Pi)/dfloat(Nky * q)                                 !Make sure to have float division 
           do j=1,Nky
               !my dispersion matrix
               if (q.eq.3) then
                  !first define dkx and stuff
                  kx = i * dkx                     !since i is in k_min = 1, k_max = Nkx (0, 2Pi] if I use same number of cores as number of Nkx
                  ky = -Pi/dfloat(q) + dfloat(j - 1) * dky           !goes like [-Pi/q, Pi/q)

                  !use p = 1 here,since use it in thesis
                  eps_full(1,1) = 2.0*t*dcos(kx) + 2.0*t2*dcos(2.0*kx);
                  eps_full(1,2) = t + 2.0*t1*dcos(kx + pi*1.0/ q) + t2* &
                        (dcmplx(dcos(dfloat(q)*ky),0.0d0)+ &
                        dcmplx(0.0d0,dsin(dfloat(q)*ky)));
                   eps_full(1,3) = t*dcmplx(dcos(dfloat(q)*ky),0.0d0)+ &
                        t*dcmplx(0.0d0,dsin(dfloat(q)*ky)) +  &
                        2.0*t1*dcos(kx - pi*1.0/ q)* &
                        (dcmplx(dcos(dfloat(q)*ky),0.0d0)+ &
                        dcmplx(0.0d0,dsin(dfloat(q)*ky))) + t2;
           
                   eps_full(2,1) = t + 2.0*t1*dcos(kx + pi*1.0/ q) + &
                        t2*(dcmplx(dcos(dfloat(-q)*ky),0.0d0)+ &
                        dcmplx(0.0d0,dsin(dfloat(-q)*ky)));
                   eps_full(2,2) = 2.0*t*dcos(kx + 2.0*pi*1.0 / q) + &
                        2.0*t2*dcos(2.0*(kx + 2.0*pi*1.0 / q));
                   eps_full(2,3) = t + 2.0*t1*dcos(kx + 3.0*pi*1.0/ q) + &
                        t2*(dcmplx(dcos(dfloat(q)*ky),0.0d0)+ &
                        dcmplx(0.0d0,dsin(dfloat(q)*ky)));
           
                   eps_full(3,1) = dcmplx(t*dcos(dfloat(-q)*ky),0.0d0)+ &
                        dcmplx(0.0d0,t*dsin(dfloat(-q)*ky)) + &
                        2.0*t1*dcos(kx - pi*1.0/ q)* &
                        (dcmplx(dcos(dfloat(-q)*ky),0.0d0)+ &
                        dcmplx(0.0d0,dsin(dfloat(-q)*ky)))+ t2;
                   eps_full(3,2) = t + 2.0*t1*dcos(kx + 3.0*pi*1.0/ q) + &
                        t2*(dcmplx(dcos(dfloat(-q)*ky),0.0d0)+ &
                        dcmplx(0.0d0,dsin(dfloat(-q)*ky)));
                   eps_full(3,3) = 2.0*t*dcos(kx + 2.0*pi*2.0*1.0 / q) + &
                        2.0*t2*dcos(2.0*(kx + 4.0*pi*1.0 / q));
